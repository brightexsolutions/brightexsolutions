...
export function TestimonialsSection() {
  ...
  return (
    <section className="py-24 bg-white dark:bg-brand-navy-dark overflow-hidden">
      ...
      >
        {testimonials.map((t) => (
          <div
            key={t.author}
            className="flex-shrink-0 w-80 sm:w-96 p-8 rounded-sm bg-brand-bg dark:bg-brand-navy-light border border-brand-border dark:border-white/10 select-none"
          >
            {/* Quote mark */}
            <div className="font-display text-5xl text-brand-gold/30 leading-none mb-4">
              &ldquo;
            </div>
            <p className="text-brand-text dark:text-white/80 text-sm leading-relaxed mb-6">
              {t.quote}
            </p>
            <div>
